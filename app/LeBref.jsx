...
function artSlug(s) { return (s || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60); }

function artLink(a) { return "/article/" + a.published_date + "/" + artSlug(a.headline); }
...
export default function LeBref() {
  ...
  useEffect(function() {
    ...
    (async function() {
      ...
      try {
        var rows = await sbFetch("articles", "published_date=eq." + selDate + "&select=id,category,subcategory,headline,tldr,summary,why_it_matters,sources,source_urls,time_published,published_date&order=time_published.desc");
        setArticles(rows);
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "'Instrument Serif', Georgia, serif", transition: "background .3s" }}>
      ...
      <header className="lb-h">
        <div className="lb-mh">
          ...
          <div className="lb-hb">
            <button className="lb-sb" onClick={toggleDark} title={dark ? "Light mode" : "Dark mode"}>
              {dark ? "\u2600" : "\u263E"}
            </button>
              ...
              {ex && <div className="lb-t2">
                ...
                <div className="lb-ft">
                  ...
                  <div className="lb-ax">
                    ...
                    <a href={artLink(a)} className="lb-sh" style={{ textDecoration: "none" }}>Permalink</a>
