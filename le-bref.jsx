...
// ============================================
const SUPABASE_URL = "https://vsqoyzkqnbyrwwnfloii.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZzcW95emtxbmJ5cnd3bmZsb2lpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQwNDYxNzIsImV4cCI6MjA4OTYyMjE3Mn0.1fHloTPEGU-bXlm5yCvLjU8FMkLVL-YIIJfgyTedFy4";
// ============================================
...
function getDateLabel(dateStr, allDates) {
...
export default function LeBref() {
    ...
    async function loadDates() {
      try {
        const rows = await supabaseFetch(
          "articles",
          "select=published_date&order=published_date.desc&limit=500"
        );
        ...
        if (unique.length > 0) {
          ...
          setLoading(false);
          setError("No articles yet. Run digest_engine.py to generate your first digest.");
        }
      } catch (e) {
        setError("Could not connect to Supabase. Check your URL and key in the code.");
        setLoading(false);
      }
    }
    loadDates();
  ...
  useEffect(() => {
    ...
    async function loadArticles() {
      ...
      try {
        const rows = await supabaseFetch(
          "articles",
          `published_date=eq.${selectedDate}&select=id,category,headline,tldr,summary,why_it_matters,sources,source_urls,time_published&order=created_at.desc`
        );
        setArticles(rows);
      } catch (e) {
  ...
  const filtered = articles.filter((a) => {
    ...
    const searchMatch =
      ...
      a.tldr.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const displayDates = availableDates.slice(0, 7);
    ...
    <div style={{ minHeight: "100vh", background: "#FAF8F5", fontFamily: "'Instrument Serif', Georgia, serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .lb-wim {
          padding: 10px 12px;
          background: linear-gradient(135deg, #FDFAF3, #FBF6EA);
        ...
        .lb-deep {
          ...
          border-radius: 20px;
          background: linear-gradient(135deg, #FDF6E8, #FBF0D8);
          transition: all 0.2s;
          white-space: nowrap;
        }
        .lb-deep:hover {
          background: linear-gradient(135deg, #FBF0D8, #F5E6C0);
      ...
      <div className="lb-content">
        {error ? (
          <div className="lb-state">
            <div className="lb-state-ico">⚠️</div>
          ...
          <>
            ...
            {filtered.length === 0 ? (
              <div className="lb-state">
                <div className="lb-state-ico">📭</div>
                    ...
                    <div className="lb-tier1" onClick={() => setExpandedId(isExpanded ? null : article.id)}>
                      <div className="lb-tier1-left">
                        <div className="lb-tier1-meta">
                          <span className="lb-badge" style={{ background: getCatColor(article.category) }}>
                            {article.category}
                          </span>
                          {article.time_published && (
                            <span className="lb-time">{article.time_published}</span>
                          )}
                        </div>
                      ...
                      <div className="lb-tier2">
                        ...
                        <div className="lb-footer">
                          <div className="lb-sources">
                            {sources.length > 0 && <>Sources: <span>{sources.join(" · ")}</span></>}
                          </div>
                          <a href={buildClaudeLink(article)} target="_blank" rel="noopener noreferrer" className="lb-deep">
